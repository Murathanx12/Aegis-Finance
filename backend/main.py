...
async def lifespan(app: FastAPI):
    ...
    async def _init_lanes():
        ...
        try:
            ...
            await asyncio.to_thread(ensure_crash2_trial)
        except Exception as e:
        ...
        try:
            from backend.services.portfolio_intelligence.h5_trial import (
                ensure_h5_trial,
            )
            await asyncio.to_thread(ensure_h5_trial)
        except Exception as e:
        ...
        try:
            ...
            await asyncio.to_thread(ensure_mom_backtest_trial)
        except Exception as e:
            logger.warning("Mom-backtest trial pre-registration failed (non-fatal): %s", e)
        try:
            from backend.services.portfolio_intelligence.trial_registry import (
                ensure_mom_trend_trial,
            )
            await asyncio.to_thread(ensure_mom_trend_trial)
        except Exception as e:
            logger.warning("Mom-trend trial pre-registration failed (non-fatal): %s", e)
        try:
            from backend.services.portfolio_intelligence.trial_registry import (
                ensure_cmp_insider_trial,
            )
            await asyncio.to_thread(ensure_cmp_insider_trial)
        except Exception as e:
            logger.warning("CMP-insider trial pre-registration failed (non-fatal): %s", e)
