...
class Strategy:
    ...
    def requires(self) -> tuple:
        return tuple(getattr(self.signal, "requires", ()))
...
def _rules() -> list:
    ...
    add(S("flow_rule_min2", "revision_flow", "the month-end rule with >= 2 firms",
          gated(col(FL), "n_firms", lo=2), caveat=_FLOW_CAVEAT))
    add(S("flow_rule_min5", "revision_flow", "the month-end rule with >= 5 firms",
          gated(col(FL), "n_firms", lo=5), caveat=_FLOW_CAVEAT))
...
NOT_REACHABLE: dict = {
    "MOM-06": "no sector/industry field on the panel",
    "MOM-07": "index-level timing rule, not a k-selection",
    "MOM-10": "beta-neutral L/S needs a short leg; the library is long-only",
    "REV-01": "daily rebalance; the library rebalances monthly",
    "REV-03": "gap_share is not built on the monthly panel yet",
    "REV-05": "short leg",
    ...
    "SIZ-04": "a 1/amihud WEIGHT rule; the engine is equal-weight only (next pass)",
    ...
    "AREV-10": "no per-analyst estimate dispersion on disk",
    "PEAD-01": "no consensus estimate at announcement",
    "PEAD-02": "no announcement-date join on the monthly panel",
    "PEAD-03": "needs PEAD-01", "PEAD-04": "needs PEAD-01", "PEAD-05": "no buyback calendar",
    "PEAD-06": "no guidance events",
    "INS-01": "routine/opportunistic insider classification not built over pit_observations",
    "INS-02": "short leg, and needs INS-01",
    "INS-03": "insider clusters not joined to the monthly panel yet",
    "INS-04": "needs INS-03", "INS-05": "needs INS-01", "INS-06": "market timing",
    "CAT-01": "no 8-K item codes on the panel", "CAT-02": "no PDUFA calendar",
    "CAT-03": "no index-change calendar", "CAT-04": "no earnings-date history on the panel",
    ...
    "COMB-04": "duplicate of AREV-06 (`mom_flow`)", "COMB-05": "needs INS-01",
    "COMB-07": "meta-router deferred: a router comes after independent selectors exist",
    "COMB-08": "needs INS-01",
    ...
    "RET-07": "no short-interest feed", "RET-08": "duplicate of MOM-04 (`hi52`)",
...
def run_strategy(panel, strategy: Strategy, *, k: int | None = None,
    ...
    for j, d in enumerate(dates):
        ...
        cost = turnover = 0.0
        if rebalance:
            top = cand[_np.argsort(-scv[cand], kind="stable")[:k]]
            new = {sym[t]: 1.0 / k for t in top}
        ...
        for s_ in held:
            ...
            rs.append(float(r) if _np.isfinite(r) else 0.0)
            if dl is not None and t is not None and dl[t]:
                n_dead += 1
        gross = float(_np.mean(rs)) if rs else 0.0
