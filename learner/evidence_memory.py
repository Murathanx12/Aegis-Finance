...
VERDICT_VOCABULARY = (
    "NOVEL", "DECAYED", "SEPARATED_NOT_SURVIVING", "SCREEN_SURVIVOR",
    "SCREEN_ONLY", "CANNOT DETERMINE", "NOISE", "REFUTED",
)
...
#: Recorded words that BIND: the export may not grade above them.
#:
#: `SEPARATED_NOT_SURVIVING` joined them on 2026-09-07 (X5). It sits ABOVE NOISE
#: in what it tells a reader -- distinguishable from zero after multiplicity --
#: and BELOW NOVEL in what it authorises: it is explicitly the word for "did not
#: clear the export bar", so an arithmetic re-derivation that reaches NOVEL must
#: not overwrite it. `_derive_verdict` never emits it (a stored observation
#: carries no family), so this entry binds only where a JOB recorded it.
_CAPPING_WORDS = ("REFUTED", "NOISE", "CANNOT DETERMINE",
                  "SEPARATED_NOT_SURVIVING")
