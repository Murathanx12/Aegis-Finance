...
434k name-months, point-in-time, split-adjusted, with the IBES 1=strong-buy
scale converted to the tracker's 5=strong-buy scale. Re-deriving that from raw
WRDS would create a second copy that drifts, so `load_names` and `load_prices`
are IMPORTED from it and the extended IBES loader here is pinned against its
`load_ibes` by `assert_ibes_loader_agrees` -- if the two ever select different
name-months, the build REFUSES rather than training on a quiet fork.

THE UNIT, SAID ONCE MORE
========================
`ratio = mean_target / close`. `upside = ratio - 1`. Both are columns; neither
is called the other. (S33b lost an afternoon to that.)
...
* (a) delisted -> the forward return is computed to the LAST observed total
  return index value and the proceeds sit in cash for the rest of the horizon.
  Dropping these rows would delete exactly the failures the horizon is meant to
  measure, which is survivorship bias with extra steps.
* (b) truncated by the panel edge -> the target is NULL. Not matured is not a
  zero and it is not a loss.

The residual risk, stated because it does not go away: CRSP `dsf.ret` carries a
delisting return only where one is on the daily file. Where it is absent, (a)
holds the last traded price, which is mildly generous to dead names.
...
SCHEMA_VERSION = "learner-train-table-1"
...
def daily_panel(px: pd.DataFrame) -> pd.DataFrame:
    ...
    tri_if_dead = last_tri.where(delisted)
...
def build(start: int = 2013, end: int = 2024, lag_days: int = 1,
    ...
    keep = ["permno", "date", "prc", "adj_prc", "high_60d", "tri", "market_cap",
    ...
    m["band"] = P.effective_band(m["ratio"], m["close"], m["coverage"]).values
    m["in_admissible"] = P.in_admissible_region(m["ratio"], m["close"], m["coverage"]).values
    ...
    for h in HORIZONS:
        ...
        m[f"delisting_filled_{h}m"] = m[f"_delistfill_{h}m"].fillna(False).astype(bool)
    ...
    cols = (["permno", "month", "vintage", "entry_date", "sector", "band",
             "in_admissible", "close", "mean_target", "market_cap",
             "schema_hash", "prior_version"]
    ...
    m = m.rename(columns={"meanptg": "mean_target"})
    ...
    # held to its last total-return index value and then sits in cash. Dropping
    # those rows would be survivorship bias; keeping them is mildly generous
    # wherever `dsf.ret` carries no delisting return. Either way the exposure is
    # a number in the receipt, not a paragraph.
