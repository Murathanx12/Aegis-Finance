...
| A6 | Backtest underperformance buried | **REAL — numbers exact** | `backend/BACKTEST_RESULTS.md`: strategy **+250.9%** vs B&H **+740.0%**, Sharpe **0.675** vs **0.921**, sell hit-rate **28.6%** vs 55% target. Not surfaced in README/UI; no `NEGATIVE_RESULTS.md`. | [M1] `NEGATIVE_RESULTS.md` written, framed per `TRACK_RECORD_POLICY` + canon A5. [VOID 2026-09-04 — the +250.9/+740.0 pair was measured against ^GSPC (price index) with 66 OVERLAPPING windows compounded as if sequential. Regenerated on SPY total return, non-overlapping: **+28.3% net vs +114.8% buy-and-hold**, Sharpe 0.432 vs 0.837, sell hit rate 0.0% of 5. The DIRECTION below stands; the magnitudes do not. See `backend/BACKTEST_RESULTS.md` and `docs/REVIEW_2026-09-04_FABLE51_VERDICTS.md` §3.1] |
