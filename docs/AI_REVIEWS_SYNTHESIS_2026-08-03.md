...
## 7b. GEMINI RESEARCH SYNTHESES (received 2026-08-03 evening — trial-design refinements)

Second deep-research batch answered the four strategy prompts. Design consequences, folded into the roadmap trials:

1. **The Quantopian benchmark (context for our whole program):** a study of 888 real strategies found in-sample Sharpe has **R² < 0.025** against out-of-sample performance, and more backtest iterations on a strategy *widen* the live gap. This is the external justification for the factory-calibration approach and the confirm-read budget: iterating against the OOS curve converts it into a training set.
2. **T1 (insider×network) — endogeneity is the pitfall to design against:** connected directors trade less but more profitably (4–7.2%/yr spread between informed-trading quintiles per the cited work), BUT centrality may proxy for individual *skill* (good traders get invited to more boards) rather than network information flow. Design fix: within-director comparison (same person's trades when more vs less connected over their career) and director fixed effects — our 17.2M role-history master supports exactly this.
3. **T2 (director departures) — adopt the "surprise departure" definition:** departures are informative when they occur while the statistical survival function for serving another year is >50% (i.e., a departure the model did not expect). Effect sizes worth the screen: +20–32% probability of earnings restatement, +29–35% probability of securities class action after a surprise departure. Our screen should model expected tenure first, flag deviations — not raw departure counts.
4. **GEX overlay — the 0DTE caveat kills the naive version:** GEX is structural (positive = dealers dampen vol, negative = dealers amplify), but models built on overnight open interest are "dangerously incomplete" now that 0DTE contracts are up to ~59% of SPX volume. Consequence: our OptionMetrics-based *historical* study is valid for pre-0DTE-era inference, but a live overlay needs intraday positioning estimates we don't have. Downgrade: measurement study on the historical era + explicit regime-validity boundary; no live overlay claim.
5. **T4/attention — measure novelty and cap-adjusted share, not volume:** a 2025 "Hype Index" line of work uses share-of-articles weighted by sector market cap to find disproportionate attention; entropy-based "unusualness" of news text predicts future volatility. Twitter attention moves Big-Tech volume/vol consistently; mainstream news and search are irregular. Consequence: T4's feature set = cap-adjusted attention share + novelty entropy + acceleration, target = future volatility.

(Caveat as always: specific citations in this batch arrived without full references — verify before the paper.)
