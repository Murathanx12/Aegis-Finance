...
Nothing was promoted, no forward book was launched, and the best rule the farm
found is **1.01x the market over 2013-2018**. The demonstrated forward edge is
unchanged at 0%.

What was built is the machine that can tell you that in an evening instead of a
quarter — and the evening was mostly the machine correcting itself. **Four
separate times the instrument moved the answer more than the answer itself:**

1. **Phase.** The first number published — `mom_12_1/h63` at $38,815 — was the
...
3. **The delisting DATA was already on disk.** `crsp__dsedelist.parquet`, in the
   ...
   into the handoff before looking — the exact failure
   `feedback_test_before_declaring_blocked` names. Joining it collapsed the
   sensitivity 18x -> **1.09x** and moved the leader $35,228 -> $80,943.
4. **Breadth read off one phase** crowned k=50; crossed with the phase, the
   optimum is **k=10** and k=5 is inside chance in three phases of five.

And then the sub-period split took most of the result back. That sequence is the
deliverable. A farm that produces a number is worth little; a farm that produces
a number and then four ways of showing the number was a draw is worth the
evening.
