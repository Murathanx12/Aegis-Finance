...
**T2, T3, T1, and the fragility composite all ✅ done (2026-06-14).**
- **Fragility composite + TRIAL-CRASH** ✅ SHIPPED (`113587f`): equal-weighted descriptive index over 8 already-fetched signals (LPPLS, SOS, Sahm, turbulence pctl, absorption ratio, net-liquidity drain, HY OAS, IG OAS); `/api/pi/fragility` → `composite` + `composite_forward_brier` (20% SPY drawdown, 30/60/90d, vs climatology) + `composite_trial`. TRIAL-CRASH pre-registered (cumulative trials 2→3; non-lane → raw floor only, N_eff unaffected). Candidate inputs **logged not asserted**: VIX term structure, options skew, **IPO issuance** (Murat's hypothesis feature — enters only as a tested candidate once sourced). HARD: descriptive-only, never arms, no "imminent" framing — grep-guard test pins it. Live read: composite **0.34 moderate**, dispersion 0.34 (turbulence/absorption elevated, LPPLS/credit calm).
- **Next — DECIDED 2026-06-14: the guarded rule-evolution loop** (Goal 2 / P1 #10) is the priority — the honest form of "run paper accounts since 2000 and learn." Full design + chunk plan in **`docs/EVOLUTION_LOOP_PLAN.md`**. Acceptance half already complete (T2's `evaluate_candidate`/effective-N); needs an orchestrator + a candidate-override hook on the existing leakage-safe `ReplayEngine`, scoped to survivorship-safe data (broad ETF + macro since ~2001 first; smart-money/individual-stock lanes wait on an as-of-constituents + SEC 13F/Form-4 data layer). **Firewall: backtests inform rules only; never the LLM.** Chunk 1 = single-param loop end-to-end.
- **Also queued:** fragility candidate inputs as *tested* features (VIX term / options skew / IPO issuance once sourced); **§P1 #6 lane framework** (needs Murat's position sizes). Follow-up research pass owed on **unverified** Section 4/5 + CSCV items before any ship as asserted.
