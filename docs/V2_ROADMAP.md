...
- **Next:** wire the candidate inputs as *tested* features (VIX term structure + options skew first; IPO issuance once a free source is sourced); then **§P1 #6 lane framework** (unblocked by T2 — needs Murat's position sizes for the mirror/conviction lanes). Follow-up research pass owed on the **unverified** items (Section 4 data-ToS, Section 5 real-time signals, CSCV/slippage mechanics) before any ship as asserted.
