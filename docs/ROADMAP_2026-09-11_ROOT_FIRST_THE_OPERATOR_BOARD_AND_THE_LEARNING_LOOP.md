...
```
NIGHT_QUEUE="B_exclusion_screen:120,N9_library_autopsy:90,C7_signal_calibration:60" python -m scripts.night_factory
```
...
| `N9_library_autopsy` | `signal` | **90 min** (resumable by cursor; a killed run continues rather than re-billing, and now writes a receipt when it is killed) | the incumbent autopsy JSONL under `research_gym/`, bars for the held panel, a reader, and — for a PAID run — a readable call ledger | `--reader local` refuses by name when no server is listening; `REFUSED_NO_LEDGER` before the first paid submission when the cap's ledger cannot be read; `REFUSED_UNPRICED_CALL` at the first flush that sees a row the price table could not price, naming the model id; `REFUSED_CAP_READER_DISAGREES` at the first flush whose cap-read and receipt-read of the same ledger differ by more than one call's worst case (22c, after run 3 spent $10.05 of $2.00 while its `cap_block` read $0.0132); the run stops at `config.N9_LIBRARY_AUTOPSY_MAX_USD` and prints the spend the CALL LEDGER reports, never a constant |
| `C7_signal_calibration` | `features` | **60 min** (the first real run took **41.5 s** over 2006-2024 for all three signals; the box is the margin for a longer panel, and a signal already written for the run date is SKIPPED, so a killed run continues) | `wrds/crsp_dsf_<year>.parquet` for the price tape, and per signal: `wrds/jkp_global_factor_usa.parquet` + `wrds/jkp_full/jkp_usa_*.parquet` (`gp_at`) and `sec_insider/insider_events_v1.parquet` (`observed_at_utc`) | a leadable signal with no point-in-time panel on disk is **NO_PANEL by name with the path it looked for**, never a guess; a signal the registry does not permit as a PICKER is excluded from the family with its grade/role; a panel too thin to test is REFUSED with the block count; a signal the box expired before is REFUSED naming the box, because an absent file would be a silence |
...
|---|---|---|
| 21 **LANDED 2026-09-20** | **EXPLOIT / EXPLORE authority split** in `decision_contract` + `roi_rank`: EXPLOIT buys only a name with a calibrated positive `μ_i` (chunk 22's output; until then the EXPLOIT set is empty and printed empty); EXPLORE takes unproven positive-EV candidates (insider t 1.4, fusion t 1.66, ...) under a fixed paper-risk budget (`config.EXPLORE_BUDGET_PCT`, default 2% of equity, 0.25% per name) allocated by Thompson sampling over each hypothesis's posterior (`exploration_score = est_alpha − costs − risk_penalty + uncertainty_bonus`). The four heuristic BUYs stop existing as a third kind of thing: each is either EXPLOIT (calibrated) or EXPLORE (budgeted) or REFUSED. **Every dollar resolves daily to `benchmark` / `active_exploit` / `active_explore` / `cash`** (his item 11) and the contract prints that split. **The morning scoreboard** (his item 12) is the first block of the pass receipt and of the desktop Ask: NAV vs SPY, exploit P&L, explore P&L, decisions made, forecasts matured/graded, calibration, strongest new positive, strongest killed, one sentence on whether anything learned changed capital. **LANDED (`b572156d` split + `51437779` scoreboard + `8abd38e2` the rebuilt contract): `backend/services/decision_authority.py`, `IC_LEGACY_HEURISTIC_SIZING = False`, `EXPLORE_BUDGET_PCT` 0.02 / `EXPLORE_PER_NAME_PCT` 0.0025 / `EXPLORE_UNKNOWN_T_SE_MULT` 3.0, `backend/services/morning_scoreboard.py`. Today's rebuilt contract: 0 EXPLOIT, 2 EXPLORE (INDV score -0.0289 rank #1, CVLG -0.8138 rank #2, 0.25% = $100 each), 38 REFUSED, 43 rows, ZERO rows with a budget and no authority. Capital resolution 99.50 / 0.00 / 0.50 / 0.00. Worst cases side by side: tilt $100,000 of $1,000,000 at the largest configured level, explore 0.50% of equity ($200 of $40,000) under a 2% ceiling, ADDITIVE. The two code items were already landed by `24f9accd` (six arms / 120, `SCENARIO_GYM_ADOPT_MIN_BLOCKS`). 30 new tests; suite green.** | `CAPITAL_CHANGED: the explore book holds CVLG and INDV on paper at 0.25% of equity each ($100 each of $40,000, 2 shares each), where yesterday CVLG was a 2.00% heuristic BUY and INDV a 0.50% WATCH; EXPLOIT holds nothing and says so; 99.50% stays in the benchmark core.` |
| 22 **LANDED 2026-09-21** | **Rank → return calibration.** For every signal `recommendation.score_candidates` can lead with, an out-of-sample map from signal decile to abnormal return AND downside at 5/21/63/126 sessions with a CI, from the panels on disk (walk-forward, date blocks, Holm over the family). Output `config`-free: a table under `backend/data/optimus/calibration/<signal>_<date>.json` that `roi_rank` reads by receipt path. Per-name `μ_i` replaces the family average. **LANDED (`9a597742` the job + `8ce070ac` the wiring): `scripts/calibrate_signal_return.py` = `C7_signal_calibration` (stage `features`, box 60 min, resumable per signal), `backend/services/signal_calibration.py` the only reader, `config.CALIB_*` + `ROI_DOWNSIDE_SOURCE = "decile_p20"` + `ROI_USE_CALIBRATION`. The family is DERIVED (adapters × registry PICKER permission) and is exactly three signals; a leadable signal with no PIT panel is NO_PANEL naming the table. First real run 41.5 s, $0.00, 2006-2024, first test year 2009, 191 month blocks, 12 Holm tests: `profitability_small` **WEAK** (D10−D1 net +0.6445%/21 sessions, gross +0.6929, cost 0.0484, NW t 1.588, Holm p 1.000, 10 non-empty deciles, Spearman **0.964**; at 126 sessions net +5.11%, t 2.78, **Holm p 0.065** — the only cell that comes near the bar); `insider_opportunistic` **WEAK** (net **−0.0463%**, gross +0.1248 against its OWN turnover cost 0.1711 — the edge is eaten by rebalancing; score 0.0 for **79.8%** of name-months so only **4 of 10** deciles are ever populated); `fusion_insider_profitability` **WEAK** (net +0.1306%, t 0.297). Both nulls behave: the end-to-end shuffled replication gives gross +0.084 / −0.029 / −0.138% at |t| < 1, and the real gross sits at the 100th / 89.5th / 99.5th percentile of the 200-draw shuffled distribution. **EXPLOIT is now gated on a CALIBRATED map**, so it can become non-empty only through measurement. Today's contract: 0 EXPLOIT, 1 EXPLORE (CVLG), 39 REFUSED + 3 agency BOOK rows; capital resolution 99.75 / 0.00 / 0.25 / 0.00; explore worst case $100 of $40,000, DOWN from $200. 37 tests; suite green.** | `CAPITAL_CHANGED: INDV's $100 of paper EXPLORE is withdrawn — its OWN decile (9 of insider_opportunistic) reads −0.0726%/month net, where yesterday it held that $100 on the family row's +0.17%/month t 1.40. CVLG keeps its $100 and its posterior moves from the family row to decile 10 of profitability_small (+0.3576%/mo, se 0.1657 from that decile's own block-bootstrap CI). HYPOTHESIS_KILLED: none INVERTED, and insider_opportunistic's D10−D1 is net NEGATIVE at 21 sessions — its gross edge is smaller than its own turnover cost. EXPLOIT remains empty and is now empty by MEASUREMENT rather than by a config row nobody could clear.` |
