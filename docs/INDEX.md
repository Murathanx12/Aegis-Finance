...
- **`ROADMAP_2026-09-25_CONNECT_WHAT_EXISTS.md` — ACTIVE (adopted 2026-09-25, Fable 5.1, plan only).**
  ...
  and NEVER-RUN ANALYST-SKILL-1 (33,043 analysts on IBES), the `RELIABILITY_PERSISTS` actor corpus, the
  committee shortlist and the investigator process all sit off that path; forecast accrual stopped 08-27
  and nothing went red; OpenClaw's spend is invisible to telemetry; the literature (Profit Mirage, Alpha
  Illusion, SoK) prescribes exactly the extractor→calibration→numeric-sizing shape §64 measured into;
  the Bloomberg challenge is CONFIRMED Oct 12–Nov 13 (register by Oct 5; WLS includes SMALL caps;
  scored on Relative P&L; a top-3% team posted +5.3%, winners +168%/+400% by variance). Lanes:
  **C** connect (receipts that can go red → run ANALYST-SKILL-1 → revision FLOW as a book → shortlist
  into `u_plan` under PROBE) · **R** reputation (shrink → floor at 0 → log-odds pool → extremize; the
  p→return calibration; the Profit-Mirage check on our own winner) · **H** the human + AI book
  (`BOOK_2026-09-25_HUMAN_AI_THEMATIC_V0.md`, drafted, Murat's veto owed, four twins) · **W** OpenClaw
  as a triggered investigator whose every quest ends in a forecast row · **K** the challenge (five
  ...
  **Night 2026-09-26: `HANDOFF_2026-09-26_THE_LIBRARY_THE_AUDIT_AND_THE_PITCH.md`** — the strategy library's first night (no DSR ≥ 0.95; momentum's hindsight CAGR collapses without its best 5 months), the since-August audit and its adjudication, the learning layer's first rules and policy_state, the PDF, the pitch.
...
- **AMENDMENT (2026-09-08, night): `ROADMAP_2026-09-08_NIGHT_ALPHA_FACTORY.md`** — two leaderboards (RESEARCH_PROVEN / PRODUCT_PROMISING) with typed statuses replace "noise" as a delete key; the $0 overnight machine (CPU queue D1→D2→G1, GPU C1 curriculum on local Qwen, G2 reads the holdout ONCE in the morning); the first hour's result: the earnings-reaction LONG leg dies in construction at 25 bps and is negative 2016-24 in every variant, the BOTTOM reaction decile keeps falling in all three eras (t −4.7) → an exit/avoid rule and a shadow put book, NOT a long hack2 book (corrects DECISIONS_2026-09-08 §4); R4's +1.80% spread carried a within-month rank look-ahead (PIT: +0.76%). Fleet: empty at the open for a fifth day, cause fixed for hack3 (a sentinel judged a direction brain; terminal `329adb5`, deploy receipt in the terminal repo). Builder prompt: `NIGHT_2026-09-08_OPUS_PROMPT.md`. **09-09 morning:** §10 (Murat's mandate: randomised windows, six armed books vs the SPY index, the past-six-months regret backtest, LLM monthly validation, the multi-network learner, Aegis Desktop, the day run); `REVIEW_2026-09-09_FABLE51_ON_THE_NIGHT_AND_THE_BACKTEST_METHOD.md` (six backtest defects; RW1: every selector's edge over the random-genome null lives in 1999-2007); day-run prompt `NIGHT_2026-09-09_OPUS_DAY_RUN_PROMPT.md`.
- **AMENDMENT (2026-09-07, later): `ROADMAP_2026-09-07_TWO_MODES_AMENDMENT.md`** — two authority levels, one system: Mode A (Murat decides, the machine grades him against four counterfactuals) ships first because it is Mode B's labelled data; nine lettered blocks X/H/F/I/E/R/U/T/S (hygiene, human mode, fleet remap by horizon, free SEC investor data, the dead news pull, Murat's era replay with a memory-free decider, the unsupervised→hypothesis route, the TradingAgents scaffold port, the Strategy interface). Builder contract: `HANDOFF_2026-09-07_FABLE51_TO_OPUS5_TWO_MODES.md` (agents on Opus 5). Why: `REVIEW_2026-09-07_FABLE51_ON_GPT_AND_MURAT.md` (GPT's summary 13/20 verified; eight places where OUR docs disagreed with their receipts; the construction-tax table mixes cost rates). Evidence appendices: `VALIDATION_2026-09-07_GPT_SUMMARY_VS_RECEIPTS.md`, `GROUNDING_2026-09-07_WEBSITE_AND_IDEAS.md`, `EXTERNAL_2026-09-07_FIVE_REPOS.md`, `EXTERNAL_2026-09-07_LANDSCAPE_WHAT_WE_MISSED.md`.
