...
| F14 | The website container runs `python /app/scripts/arena_paper_repair_once.py` on every start → "No such file or directory". The script IS tracked in git (`scripts/arena_paper_repair_once.py`) but `backend/Dockerfile:18-19` copies only `backend/` and `engine/`, never `scripts/` — so a command set in the Railway service settings (not in `railway.json`) points at a file the image cannot contain, and fails silently on every start. | Railway service config + `backend/Dockerfile:18-19` | `railway logs -s Aegis-Finance` |
