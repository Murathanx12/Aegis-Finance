...
| 🔵 T9 | Insider opportunistic-buy signal (item 3) as a FORWARD IC trial. **Signal + data source built & tested 2026-06-16:** `compute_opportunistic_buy_score` (open-market `P` only, distinct-buyer cluster + tanh($) bonus) + `insider_form4.py` (raw SEC Form 4 XML, hang-proof — **Finnhub free lacks code/price; edgartools hung ~50min, both rejected**). 12 offline tests. Pre-registered: `TRIALS/TRIAL-INSIDER-IC.md` (estimator = forward rank-IC @ 21/63/126d). **Remaining:** wire weekly PIT collector (book ∪ small-cap watchlist) → starts the forward clock; do with/after seed. Descriptive until forward IC proven. | signal done; collector pending seed |
