...
| 🔵 TX | TRIAL-EXIT ATR exit-overlay lane (item 6 risk layer). **Decision core DONE 2026-06-17:** `exit_overlay.py` — `evaluate_exit_overlay` (per-position ATR Chandelier hold/exit) + `vol_capped_weights` (vol-target sizing), reusing `exit_engine`. 6 tests. Checklist item 1 complete; **items 2–4 ATTENDED** (separate-hash `conservative-atr` config, `run_exit_overlay_check` wired no-op-until-seeded à la Plan 3, env-gated seed `AEGIS_SEED_CONSERVATIVE_ATR`, register TRIAL-EXIT). See `TRIALS/TRIAL-EXIT-atr-trailing-stops.md`. | core done; lane = attended seed |
