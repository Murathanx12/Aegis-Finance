...
| ⬜ T12 | External-investor decision feeds, phase 2 (solving "no users → no decision data" with public sources) — candidates in priority order: **ARK-style daily ETF holdings diffs** (real manager decisions, daily, free CSVs), **N-PORT monthly fund holdings** (EDGAR), **13F superinvestor clone scores** (collector exists, unscheduled), **Chen–Zimmermann 212-signal harness** (survivorship-clean direction-check, report item 7). Each = its own pre-registered trial; same PIT/fail-loud pattern as T11. | open |
