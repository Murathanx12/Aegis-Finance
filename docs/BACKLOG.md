...
| 🔵 T8 | Multi-factor selection model (item 5/3b). **Built & wired 2026-06-17:** `multifactor.compute_multifactor_scores` (z-score each factor → equal-weight mean) over momentum (live, leak-free) + insider + revisions (from PIT); composite snapshotted `multifactor_score:{ticker}`, weekly, in `_daily_check` after the two collectors. **Quality (Piotroski) DEFERRED** — edgartools hang risk; one-line add once a guarded fundamentals path exists. Generic `pit_score_collector` shared w/ T10. 7 tests. Pre-reg `TRIALS/TRIAL-MULTIFACTOR-IC.md`. Forward IC clock starts next deploy. Reframed by T7: forward only, never a survivor backtest. | collector wired; IC accruing |
...
| 🔵 T11 | **Congressional trades forward-IC** (external-investor decision data, phase 1 — Murat's "no users → import public decisions" idea). Built & wired 2026-07-11: `congress_trades.py` (FMP `senate-latest`+`house-latest`, PIT by **disclosureDate**, fail-loud incl. contract-drift guard; the free GitHub stock-watcher dumps found DEAD since 2021 and rejected) + `congress_collector.py` → `congress_score:{ticker}` (net distinct-member purchases over 90d of disclosures), dynamic ~150-name universe (first non-book cross-section — also serves "widen the universe"), weekly in `_daily_check`, registry row via `ensure_congress_trial` at startup. 15 tests. Pre-reg `TRIALS/TRIAL-CONGRESS-IC.md`. Honest prior: post-STOCK-Act edge likely faded. **Needs `FMP_API_KEY` set on Railway.** | collector wired; IC starts next deploy |
| 🔵 T12 | External-investor decision feeds, phase 2. **ARK daily holdings SHIPPED 2026-07-11:** `ark_holdings.py` (6 fund CSVs verified live, fail-loud on header drift/empty; MM/DD/YYYY→ISO) + `ark_collector.py` (daily raw `ark_shares:{fund}:{ticker}` snapshots as_of file date; frozen 21-session flow score `ark_score:{ticker}` SELF-ARMS once baseline accrues ≈ mid-Aug — no false-neutral early zeros), wired in `_daily_check`, registry row via `ensure_ark_trial`, 13 tests, pre-reg `TRIALS/TRIAL-ARK-IC.md` (honest prior: null/negative — copy-ARK literature). 13F: already wired 2026-07-08 (not "unscheduled"). Remaining: **N-PORT monthly holdings** (EDGAR), **Chen–Zimmermann harness**. | ARK live; N-PORT/CZ open |
| ⬜ U1 | **Merge /crash + /simulation into /outlook** (UX audit 2026-07-11): /crash is ~90% a duplicate of /outlook (unique: 60-month CrashTimelineSection + horizon toggle); /simulation adds only interactive sim-count/horizon controls. Move the unique pieces into Outlook, redirect both routes, extract the copy-pasted CrashGauge/ShapChart/TickerCrashCard into components/. Sidebar was regrouped + palette entries added 2026-07-11; this merge is the remaining 'items repeat' fix. | open |
| ⬜ U2 | **Parallelize `_analyze_stock` enrichments** (perf audit 2026-07-11): ~15 per-ticker service calls run sequentially (options, earnings, insider, TA, liquidity, drawdown, valuation, dividends, vol, patterns, LPPL, trends) ≈ 40-60s of serialized I/O → ThreadPoolExecutor (pattern exists in `_screener._analyze_one`) → ~8-12s. Prereq for warming per-ticker watchlist keys. Shared market signal + screener warm shipped 2026-07-11. | open |
| ⬜ T13 | **Pre-register a max-growth PAPER lane** (own YAML + hash, seed-a-lane attended) mirroring the new `max_growth` builder template (VTI 30/QQQ 35/MTUM 20/VGT 15; direction-checked 3y CAGR 26.5% vs SPY 20.7%, 2015→ 17.1% vs 13.5%, DD ≤ SPY both windows) so the tilt earns a FORWARD record instead of a backtest claim. Murat flips the seed flag. | open |
| 🔵 TX | TRIAL-EXIT ATR exit-overlay lane (item 6 risk layer). **LANE FULLY WIRED 2026-06-17:** decision core (`exit_overlay.py`, 6 tests) + the live `conservative-atr` lane — `conservative_atr_lanes.yaml` (own hash, distinct from ref+book), `exit_lane.py` (`seed_conservative_atr_lane` registry-on-seed + `run_exit_overlay_check` wired no-op-until-seeded à la Plan 3: stop→cash + vol cap), `main.py` env-gated `AEGIS_SEED_CONSERVATIVE_ATR`, MTM + nav_freshness + track-record + registry N_eff all skip-until-seeded. 9 lane tests (hash isolation, no-op, idempotent, registry-on-seed, frozen control untouched, overlay fires). **NOTE: a 2026-06-17 seed attempt was a no-op because the wiring didn't exist yet — now it does.** Remaining: Murat re-flips `AEGIS_SEED_CONSERVATIVE_ATR=1` (cumulative trials → 6). See `TRIALS/TRIAL-EXIT-atr-trailing-stops.md`. | wired; ready to seed |
...
- **⚠️ REMAINING (separate task, NOT M3): crash-model discrimination.** The retrained
  ...
  lane per TRIAL-001.
- **2026-07-11 re-confirmation + path forward:** a fresh retrain reproduced the
  degeneracy exactly (AUC unmeasurable — zero crash events in the purged val
  window; 6m head emits ONE value; 12m 43% ≈ 3× base rate; only 5 features
  survive LASSO) → **deploy HELD**, `model_not_deployed` in prod is the honest
  state (NEGATIVE_RESULTS §6). The binary ≥20%-drawdown label is unlearnable on
  one market's history. Successor plan (docs/research/
  CRASH_AND_OSS_RESEARCH_2026-07-11.md): P1 consolidate crash logic into
  `backend/services/crash/` + wire near-term forward spread & excess-bond-premium
  candidates + STLFSI4/NFCI as benchmarks-to-beat; P2 retrain on forward
  max-drawdown SEVERITY (quantile trees → multi-threshold exceedance probs,
  PR-AUC + event-window eval — the AUC≥0.70 gate is unmeasurable for this label);
  P3 forward-only promotion via pre-registered trial. The `walk-forward AUC≥0.70`
  line in CLAUDE.md's healthy ranges is superseded for the crash model.
