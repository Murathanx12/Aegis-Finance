...
| B2 hold | OPEN — **entry gate DONE (variables + local commit); five items promoted to the top on evidence** | The entry pass has NO deadline gate (`run_pass.py` vs `exits.py:114`) and `--manage-only` is documented but never emitted, so the fleet churned post-expiry today: hack2 opened and lost a PANW short in 24 seconds; hack1 held an unfilled ~$16.9k short in a SAFE account. Also: `exits.py`'s flat 3% stop pre-empts the correctly-profiled venue stop (8% hack3 / 6% hack4) while the same un-profiled 3% understates the sizing worst case ~2.5x, and the re-entry guard fails OPEN on exception. **Fixed at the variable layer 09-04/05** (`--manage-only` on all six + `AAT_ENTRY_STYLE` deleted on hack4/hack6, which closes the pre-open `open_auction` path that runs BEFORE the manage-only check at `agent_loop.py:330` vs `:347`); structural gate + `Mandate.manage_only` in terminal `fd0c75b`, local, **needs a push**. NEW, from `FINDING_2026-09-05_THE_MEGA11_EXEMPTION.md`: (a) refuse when `expected_edge_usd < k x stop_loss_usd` — all five PANW shorts were **1:8 against** ($92-128 edge vs a $700+ stop) and negative-EV before any signal question; (b) the mega-11 PEAD path was never given the 2026-08-26 amendment (scoped *"outside the mega-11"*) so it still trades an unhedged short from a beta-hedged log-return measurement — **t 2.82 becomes t 1.45 raw simple**, and PANW's DOWN-big cell is **−0.852% t −0.59**; (c) `Mandate.tier` is DECLARATIVE ONLY — nothing outside `fleet.py` reads it, so a SAFE book opened an unbounded short by omission; (d) `exits.manage` cancels a stop then closes, and never re-places the stop when the close fails (hack2 was naked-short 76 min); (e) declare `manage_only` on hack2, which has the same brain and `universe="window"`. Timing: `PROPOSAL_2026-09-05_DECISION_TIMING_NOT_POLLING.md` — event triggers replace the 30-min entry clock, but **only after B2 §1-3**, and no compiled-in permanent stops (Murat, 09-05: safety states stay instantly flippable at the variable layer) |
| B3 loop | blocked on B2 §5 | — |
| B4 inference | blocked on B1 §3 | — |
