...
def ledger_spend_for_this_job() -> dict:
    """Total wire spend for this job's purpose, DERIVED FROM THE LEDGER.

    A `--replay` costs nothing, and the first version of this receipt therefore
    reported `llm_spend_usd: 0.0` after a replay -- the record of what the round
    actually cost was overwritten by the act of reproducing it. Telemetry that a
    reproduction can erase is not accounting, so the total is read back from
    `llm_research.LEDGER_PATH` every time, by purpose, and the per-call output
    hashes come with it.
    """
    from backend.services import llm_research as LR
    path = LR.LEDGER_PATH
    if not path.exists():
        return {"wire_calls": 0, "wire_spend_usd": 0.0,
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        try:
            ...
            continue
        if r.get("purpose") == "growth_g3_mutations":
            calls.append(r)
    return {
        "ledger": str(path),
        "wire_calls": len(calls),
        "wire_spend_usd": round(sum(float(c.get("cost_usd") or 0.0) for c in calls), 6),
        ...
        "output_sha256_per_call": [c.get("output_sha256") for c in calls],
        "note": ("DERIVED from the ledger by purpose, so a --replay cannot "
                 "understate what the round cost."),
    }
...
def run(*, dry_run: bool = False, replay: bool = False,
    ...
    out = {
        ...
        "llm": {**spend, "prompt_chars": len(prompt),
                "wire_spend_across_all_rounds": ledger_spend_for_this_job(),
                "cap_usd": LLM_CAP_USD,
        ...
        "mutation_rounds": [
            ...
            {"round": 2, "wire_call": True, "spend_usd": 0.004101,
             "output_sha256": "a6873a6d70bb91ef",
             ...
             "receipt": None},
            {"round": 3, "wire_call": True, "spend_usd": 0.004492,
             "output_sha256": "c00519899b5381e7",
             "spend_note": ("the WIRE spend of round 3. This receipt may have "
                            "been regenerated by --replay at $0.00 since; the "
                            "round still cost what it cost, and the total is "
                            "derived from the ledger in `llm.wire_spend_across_"
                            "all_rounds`."),
