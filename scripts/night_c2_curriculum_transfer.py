...
def _daily_returns(df_te, pred, label):
    ...
    for day, g in t.groupby("entry_date", sort=True):
        ...
        cost = 2.0 * (COST_BPS_PER_SIDE / 1e4) * 2.0   # both legs, in and out
        out.append({"date": day, "gross": gross, "net": gross - cost, "n": len(g)})
...
def run(smoke=False, label="x_oc", n_folds=5) -> dict:
    t0 = time.time()
    rng = np.random.default_rng(SEED)
    cur_texts, cur_y, cur_kind = load_curriculum(max_docs=400 if smoke else None)
    ...
    for tr_days, te_days in _folds(dates, n_folds):
        ...
        for arm, X in feats.items():
            ...
            head.fit(X[tr], y[tr])
            series[arm].append(_daily_returns(panel[te], head.predict(X[te]), label))
    ...
    for arm, parts in series.items():
        ...
        per_arm[arm] = {"gross": _stat(d["gross"]), "net": _stat(d["net"]),
    ...
    receipt = {
        ...
        "construction": {"decile_long_short_equal_weight": True,
                         ...
                         "cost_note": "a flat 100 bps per date, i.e. full daily turnover on "
                                      "both legs. It is a CONSTANT and identical across arms, "
                                      "so it moves every level and no ranking; read gross for "
                                      "the size of the effect and net for whether it survives "
                                      "a book that re-ranks every session",
        ...
        "headline": (f"TRANSFER {tr_a}%/yr net vs shuffled-label CONTROL {ct_a} vs "
                     ...
                     f"{panel['entry_date'].nunique()} dates"
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    print(json.dumps(run(smoke=a.smoke, label=a.label, n_folds=a.folds), indent=1, default=str))
